...
struct_dir = 'datasets'
database_dir = 'database_gsv_vga'
queries_dir = '247query_subset_v2'

# test images in tokyo247, 1024 x 768,  768 x 1024
# CNN is susceptible to aspect ratio = H/W
def input_transform():
...
def get_whole_training_set(root_dir, onlyDB=False):
    return WholeDatasetFromStruct(root_dir,
                                  structFile='tokyoTM_train.mat',
                                  input_transform=input_transform(),
                                  onlyDB=onlyDB)

def get_whole_val_set(root_dir, onlyDB=False):
    return WholeDatasetFromStruct(root_dir,
                                  structFile='tokyo247.mat',
                                  input_transform=input_transform(),
                                  onlyDB=onlyDB)

def get_training_query_set(root_dir, margin=0.1):
    return QueryDatasetFromStruct(root_dir,
                                  structFile='tokyoTM_train.mat',
                                  input_transform=input_transform(), margin=margin)

def get_val_query_set(root_dir):
    return QueryDatasetFromStruct(root_dir,
                                  structFile='tokyoTM_val.mat',
                                  input_transform=input_transform())
...
def parse_dbStruct(path):
    ...
    # get the query image 7416 for pitts30k
    qImage = [f[0].item() for f in matStruct[3]]
    # get the utm coor for query image [7416x2]
    utmQ = matStruct[4].T

    # number of DB = 10000
    numDb = matStruct[5].item()
    # number of query = 7416
    numQ = matStruct[6].item()

    # pos distance thres: 25m
    posDistThr = matStruct[7].item()
    # dis thre squ = 25^2 = 625
    posDistSqThr = matStruct[8].item()
    # non-trivial pos dis thre square = 10^2 = 100
...
class WholeDatasetFromStruct(data.Dataset):
    def __init__(self, root_dir, structFile, input_transform=None, onlyDB=False):
        ...
        self.images = [join(self.database_dir, dbIm[:-3]+('png')) for dbIm in self.dbStruct.dbImage]
        if not onlyDB:
...
class QueryDatasetFromStruct(data.Dataset):
    def __init__(self, root_dir, structFile, nNegSample=1000, nNeg=10, margin=0.1, input_transform=None):
        super().__init__()
    ...
    def __getitem__(self, index):
        ...
        positive = Image.open(join(self.database_dir, self.dbStruct.dbImage[posIndex]))
        ...
        for negIndex in negIndices:
            negative = Image.open(join(self.database_dir, self.dbStruct.dbImage[negIndex]))
            if self.input_transform:
