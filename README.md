...
To run the demo, copy the checkpoints that training code produce to the `/resources` folder and rename the hardcode path for loading checkpoints in the python script. Then cd to the demo file and run command `python app.py`. This would run the flask application for the website. Then go to `localhost:5000` on your browser, you should be able to see an image like this
